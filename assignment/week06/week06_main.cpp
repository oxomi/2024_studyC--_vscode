	...
	while( true )
	{
		...
		switch( mainNum )
		{
			...
			case 3 :
				...
				{
					(*iter)->Action(); //여기서 화살표 연산자를 사용해야 하는 이유는..? == (*iter) 는 객체가 아니라 주소
				}
...
//중괄호를 붙여주니까 바로 정상작동 되었다. 선언하지 않고 Animallist.push_back(new week06::Cat(tempName, tempNum)); 만 하면 중괄호 안해도 괜춘.
